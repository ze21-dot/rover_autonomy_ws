...
run () {   # name model mode speed lane rocker_kd
  local name=$1 model=$2 mode=$3 speed=$4 lane=$5 rkd=${6:-900}
  local d=/workspace/runs/$name
  rm -rf "$d"; mkdir -p "$d"
  echo "=== $name  start $(date +%H:%M:%S)  model=$model mode=$mode speed=$speed lane=$lane"
  RUN=$name MODEL=$model MODE=$mode SPEED=$speed LANE=$lane ROCKER_KD=$rkd \
    timeout 1200 python /workspace/mars_traverse.py > "$d/log.txt" 2>&1
  echo "  (exit $?)"
  ffmpeg -y -loglevel error -framerate 24 -pattern_type glob -i "$d/frames/rgb_*.png" \
...
run B_passive_7        revolute  passive  7      1     2000
run C_hybrid_7         revolute  hybrid   7      1     900
...
run C_hybrid_13        revolute  hybrid   13     1     900
